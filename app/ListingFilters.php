    ...
    public function acres($filter)
    {
        if (env('APP_CLIENT') == 'oregon') {
            ...
            if (count($filter_value) == 2) {
                return $this->builder->where('acres', '>', $filter_value[0] + 0)->where('acres', '<',
                    $filter_value[1] + 0);
            } else {
            ...
            return $this->builder->whereBetween(DB::raw('(cast(lot_size as decimal(20, 2)) / 43560)'),
    ...
    public function pool($filter = [])
    {
        if (env('APP_CLIENT') == 'iowa') {
            ...
            if (env('APP_CLIENT') == 'hawaii') {
                ...
                if (env('APP_CLIENT') == 'connecticut' || env('APP_CLIENT') == 'oregon' || env('APP_CLIENT') == 'phoenix') {
                    foreach ($filter as $value) {
...
//                    ->orWhereIn('listings.area', $filter);
    ...
    public function subdivisionIds($filter)
    {
        ...
        if (count($filter)) {
            $subdivisionsNames = Subdivision::query()->select('name')->whereIn('id', $filter)->get()->toArray();
...
//    public function construction($filter)
//    {
//        return $this->builder->where('construction_status', 'like', '%' . $filter . '%');
//    }
}

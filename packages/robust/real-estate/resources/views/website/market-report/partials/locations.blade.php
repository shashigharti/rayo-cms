...
<div class="row">
    ...
    @elseif(($page_content == 'market-report') )
        <div id="market__search--lists" data-page-type="{{$page_type}}" data-insight-url="{{url('market/reports/in')}}" class="market__search--lists market--right__search col m10 s12">
            @foreach($records as $report)
                <div class="col market__search--lists-item--single">
                    <div class="market__search--lists-item card">
                        <div class="card-content">
                            <p data-id="{{$report->reportable->id}}" data-type="Title" data-value="{{$report->reportable->name}}" data-url="{{route('website.realestate.market.reports.in', [
